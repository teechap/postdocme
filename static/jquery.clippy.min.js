...
*/
(function(b){var a={width:14,height:14,clippy_path:"clippy.swf",keep_text:false,force_load:false,flashvars:{},swfobject_attributes:{}};b.fn.clippy=function(f){f=b.extend(true,a,f);var c=false;try{var d=new ActiveXObject("ShockwaveFlash.ShockwaveFlash");if(d){c=true}}catch(g){if(navigator.mimeTypes["application/x-shockwave-flash"]!=undefined){c=true}}if(c||f.force_load){b.each(b(this),function(e,k){var j="";if(typeof f.text!="undefined"){j=f.text}else{if(b(k).data("text")&&b.trim(b(k).data("text"))!=""){j=b(k).data("text")}else{j=b.trim(b(k).text())}}j=encodeURIComponent(j);var l="";if(typeof b(k).attr("id")==="undefined"||b.trim(b(k).attr("id"))===""){var i=Math.round(Math.random()*10240).toString(16);l="clippy-"+i;b(k).attr("id",l)}else{l=b(k).attr("id")}if(!f.keep_text){b(k).html("")}var h=b.extend({},f.flashvars,{text:j});swfobject.embedSWF(f.clippy_path,l,f.width,f.height,"10",false,h,{scale:"noscale"},f.swfobject_attributes)})}else{b.each(this,function(e,h){b(h).css({display:"none"})})}}})(jQuery);